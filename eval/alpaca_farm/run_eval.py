...
assert type(IS_ALPACA_EVAL_2) == bool, "IS_ALPACA_EVAL_2 should be a boolean."

def main(args):
    if args.no_evaluate_with_llm:
        logging.info("\n\nPlease note that the evaluation is not done with LLM.")
        logging.info("We will only generate the outputs and save them to the save_dir.\n\n")
    ...
    annotators_config = "weighted_alpaca_eval_gpt4_turbo" if IS_ALPACA_EVAL_2 else "alpaca_eval_gpt4"
    logging.info(f"annotators_config: {annotators_config}")
    
    logging.info(f"Enironment variables IS_ALPACA_EVAL_2: {IS_ALPACA_EVAL_2}")
    ...
    if os.path.exists(os.path.join(args.save_dir, f"{model_name}-greedy-long-output.json")):
        with open(os.path.join(args.save_dir, f"{model_name}-greedy-long-output.json"), "r") as fout:
            model_results = [json.loads(line) for line in fout]
    else:            
        prompts = []
    ...
    args.save_dir = args.save_dir + "_v1" if not IS_ALPACA_EVAL_2 else args.save_dir
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_evaluate_with_llm",
        action="store_true",
        help="If not given, we will evaluate the model with LLM.",
    )
    parser.add_argument(
        "--save_dir",
        type=str, 
