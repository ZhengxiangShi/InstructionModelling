    ...
    for i in range(0, len(prompts), batch_size):
        ...
        try:
            ...
            if stop_id_sequences:
                for output_idx in range(batch_outputs.shape[0]):  # (zshi) For each example in the batch
                    for token_idx in range(batch_input_ids.shape[1], batch_outputs.shape[1]):  # (zshi) For each generated token in the output
                        if any(batch_outputs[output_idx, token_idx: token_idx+len(stop_sequence)].tolist() == stop_sequence for stop_sequence in stop_id_sequences):
                            batch_outputs[output_idx, token_idx:] = tokenizer.pad_token_id  # (zshi) Set all tokens after the stop sequence to pad token
                            break
    ...
    for i in range(0, len(unrolled_examples), batch_size):
        ...
        for example_idx, (prompt, example) in enumerate(zip(batch_prompts, batch_examples)):
            ...
            if tokenizer.padding_side == "right":
                ...
                example_logits = outputs.logits[example_idx, -len(tokenized_example):, :]  # (zshi) For left padding, we need to remove the first few logits, which are for the padding tokens.
            ...
            completion_logits = example_logits[len(tokenized_prompt)-1:len(tokenized_example)-1, :]  # (zshi) Get the logits for the completion portion, excluding the last token, which is the padding token.
