...
from utils import compute_kl_divergence_loss, logprobs_from_logits
...
def add_ref_model_logit_to_columns(example, ref_model_logit):
    example['ref_model_logits'] = ref_model_logit
    return example
...
def main():
    ...
    if args.apply_kl_div_loss:
        ...
        for _, batch in tqdm(enumerate(train_dataloader), total=total_len, desc="Computing reference model logits"):
            with torch.no_grad():
                batch = {k: v.to(ref_model.device) for k, v in batch.items()}
        ...
        train_dataset = train_dataset.add_column("ref_model_logits", ref_model_logits)
        del ref_model, train_dataloader
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                if args.apply_kl_div_loss:
                    ref_outputs_logits = batch.pop("ref_model_logits")
                    ref_outputs_logits = torch.tensor(ref_outputs_logits, device=model.device)
                    # with torch.no_grad():
                    #     ref_outputs = ref_model(**batch.cpu().to(ref_model.device), use_cache=False)
                ...
                if args.apply_kl_div_loss:
                    kl_loss = compute_kl_divergence_loss(
                        ...
                        ref_logits=ref_outputs_logits,
                        input_ids=batch["input_ids"],
                        labels=batch["labels"],
                        kl_penalty=args.kl_penalty_type,
                    )
